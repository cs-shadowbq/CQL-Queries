...
## Formatting and Selecting Specific fields for a Table after Aggregation
...
Dashboards live in `./dashboards/` as YAML files conforming to `https://schemas.humio.com/dashboard/v0.23.0`.
...
Widget `x`/`y` coordinates are absolute within the full 12-column dashboard grid, **not** relative to the section. Sections are purely for display grouping (collapsible) and optional per-section time override.
